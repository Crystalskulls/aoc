...
func main() {
	...
	lastWinnerBoard, lastNumber := play(bingoNumbers, bingoBoards)
	if lastWinnerBoard == nil {
		log.Fatal("lastWinnerBoard is nil")
	}
	sum := sumUnmarkedNumbers(lastWinnerBoard)
	fmt.Println(lastWinnerBoard)
	fmt.Printf("%d * %d = %d\n", sum, lastNumber, sum*lastNumber)
}

func play(bingoNumbers []int, bingoBoards []*BingoBoard) (lastWinnerBoard *BingoBoard, lastNumber int) {
	remaining := len(bingoBoards)
	for _, bingoNumber := range bingoNumbers {
		for _, bingoBoard := range bingoBoards {
			...
			if bingoBoard.bingo {
				remaining--
				if remaining == 0 {
					lastWinnerBoard = bingoBoard
					return lastWinnerBoard, bingoNumber
				}
			}
		}
	}
...
func (bingoBoard *BingoBoard) check(bingoNumber int) {
	for _, row := range bingoBoard.rows {
		for j, number := range row.numbers {
			...
			if bingoBoard.bingo == false && (row.markedNumbers == 5 || bingoBoard.columns[j].markedNumbers == 5) {
