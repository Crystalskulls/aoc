...
func main() {
	...
	for len(transmissionSystem.binaryStream) != 0 {
		...
		if isLiteralValue(typeID) {
			...
			switch typeID {
				case 0:
					result += transmissionSystem.sum(lengthTypeID)
				default:
			}
		}
	}
...
func (transmissionSystem *TransmissionSystem) sum(lengthTypeID int) (result int) {
	if lengthTypeID == 0 {
		lengthBits, _ := transmissionSystem.read(15)
	} else {
		subPackets, _ := transmissionSystem.read(11)
	}
}
